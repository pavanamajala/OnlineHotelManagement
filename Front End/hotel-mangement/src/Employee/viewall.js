...
function ViewAllEmployee(){
  ...
  const loadUsers=async()=>{
      const result=await axios.get("http://13.49.68.81:8888/user/employee/viewall",{headers:Auth()});
      setUsers(result.data);
  }

  const deleteUser=async (employeeId)=>{
      await axios.delete(`http://13.49.68.81:8888/user/employee/delete/${employeeId}`,{headers:Auth()})
      loadUsers()
  }
