...
export default function UpdateEmployee() {
    ...
    const onSubmit=async (e)=> {
        e.preventDefault();
        await axios.put(`http://localhost:8888/user/employee/update/${employeeId}`, user, {headers:Auth()});
        navigate("/user/employee/viewall")
    };

    const loadUser=async()=>{
        const result=await axios.get(`http://localhost:8888/user/employee/view/${employeeId}`, {headers:Auth()});
        setUser(result.data);
    }
