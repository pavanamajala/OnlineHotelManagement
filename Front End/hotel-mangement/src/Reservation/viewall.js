...
function ViewAllReservation(){
  ...
  const loadUsers=async()=>{
      const result=await axios.get("http://13.49.68.81:8888/reservation/viewall");
      setUsers(result.data);
  }

  const deleteUser=async (reservationId)=>{
      const del=await axios.delete(`http://13.49.68.81:8888/reservation/delete/${reservationId}`)
      alert(del.data)
      loadUsers()
  }
